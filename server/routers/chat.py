...
def _quick_classify(query: str, last_shown: list = None) -> dict | None:
    ...
    if re.match(r'^[\w\s一-鿿]{2,20}$', msg) and not any(
...
async def _llm_classify_intent(query: str, doubao_service=None, last_shown: list = None) -> str:
    ...
    返回：'search' | 'compare' | 'scene' | 'clarify'
    """
...
async def chat_stream(request: ChatRequest, http_request: Request):
    ...
    async def generate():
        try:
            ...
            # 对 scene/compare 意图跳过 analyze_query（结果不会被使用）
            if quick and quick.get('intent') in ('scene', 'compare'):
            ...
            # 意图判定
            if quick and quick['intent'] == 'compare':
                intent = 'compare'

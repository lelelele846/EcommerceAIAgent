"""
Prompt 构建模块 — 组装对话上下文、商品信息和人格设定为完整 prompt。

核心构建块：
    - _build_memory_section：会话记忆区（摘要、偏好、意图链）
    - _build_conversation_history：最近对话历史
    - _build_last_shown_section：上一轮展示商品（防止追问断裂）
    - _build_personality_prompt：人格设定（闺蜜风格）
    - _detect_emotion / _detect_user_mode：用户状态分析
    - _extract_life_events：生活事件提取
    - build_prompt：主入口，整合所有区块为最终 prompt

特色功能：
    - Token 节省：使用会话摘要而非完整历史
    - 情感识别：检测用户负面情绪并给予共情回应
    - 用户模式判断：区分"目的型"和"闲逛型"用户
    - 上下文衔接：明确告知 LLM 追问是在筛选上一轮结果
"""
...
7. **区分「精准匹配」与「相关推荐」**：
   - 如果商品信息中有品类完全对口的商品（如用户要遮阳帽，列表里有遮阳帽），把它们作为主要推荐
   - 如果对口商品较少（≤2 款），可以在主要推荐后加一行「💡 你可能还需要的好物：」，再列出同品类但不完全对口的商品
   - 两种商品不能混在一起推荐，必须在文案中明确区分
8. **当商品列表中完全没有用户要的品类时**：先说明"目前咱们暂时还没有{{用户要的品类}}呢，不过帮你找了几款相关的～"，再推荐，绝不要只说一句"没有"就结束

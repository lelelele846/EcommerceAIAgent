...
| 多模态 | 拍照找货（VLM + 向量双路径）、TTS 语音播报 |
| 工程 | 检索结果缓存、会话隔离、SQLite 持久化、流式 SSE 推送 |
...
| Embedding | Doubao-embedding-vision（图像向量化，2048 维） | — |
| 本地 Embedding | all-MiniLM-L6-v2（SentenceTransformers，文本检索 384 维） | 2.6.1 |
| 向量库 | Chroma | 0.4.24 |
...
| 多模态 | ActivityResult API / 系统相机 / TextToSpeech |
| 序列化 | Gson 2.10.1 |
...
端侧拍照/相册选图
    ...
    → Base64 编码
    → 后端双路径自动切换：
       ├─ 向量路径：Doubao-embedding-vision 图像向量化 → Chroma 向量检索
       └─ VLM 降级：向量检索 0 匹配时自动回落 Doubao VLM 识别 → 文本 RAG
    → 返回视觉相似商品
```
...
用户的反问答复通过 `pending` 机制确定性解析（不经 LLM），点击即可精准识别；同时支持输入"直接帮我搜"随时退出反问。

### 亮点四：多模态全链路——从拍照到语音的完整交互闭环

- **拍照找货**：向量检索 + VLM 降级双路径，自动容错切换
